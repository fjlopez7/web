...
import { GET_PLAYBOOK_ID, EDIT_PLAYBOOK } from '../graphql/PlaybookMutations';
...
export const EditPlaybook = (props: any) => {
    ...
    const {loading: loading1 , error: error1, data: playbook} = useQuery(GET_PLAYBOOK_ID, {
        client: clientPlaybooks,
    ...
    useEffect(() => {
       if (!loading1){
            setTitle(playbook.getPlaybookById.title)
            
            setUnits(playbook.getPlaybookById.units)
            setKeywords(playbook.getPlaybookById.keywords)
            playbook.getPlaybookById.keywords.forEach(function (key:  string){
                let keys = keywords
                keys.push(key)
                setKeywords(keys)
            })
            playbook.getPlaybookById.units.forEach(function (id: string){
                let play = data.getAllPlays.find((element: any)=> element.id === id )
                let plays1 = plays
                plays1.push(play.title)
                setPlays(plays1)
                let units1= units
                units1.push(id)
                setUnits(units1)
    ...
    const [editPlaybookMutation] = useMutation(EDIT_PLAYBOOK,  {
        client: clientPlaybooks,
      }
    );
    ...
    const handleSubmit = (e: any) =>{
        ...
        if (title === ''){
            ...
            editPlaybookMutation({
                variables: {id: props.match.params.id, title: title, units: units, keywords: keywords, creator: userId } 
            }).then(response => {
                setTitle('')
                setUnits([])
                setPlays([])
